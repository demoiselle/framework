...
public class SecurityFilter implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext req) throws IOException {
        if (req.getMethod().equals("OPTIONS")) {
            ...
            if (config.isCorsEnabled()) {
                config.getParamsHeaderSecuriry().forEach((key, value) -> responseBuilder.header(key, value));
    ...
    private void applyCorsHeaders(ContainerRequestContext req, Response.ResponseBuilder responseBuilder) {
        List<String> allowedOrigins = config.getCorsAllowedOrigins();
        if (allowedOrigins == null || allowedOrigins.isEmpty()) {
            config.getParamsHeaderCors().forEach((key, value) -> responseBuilder.header(key, value));
            return;
        }
        ...
        responseBuilder.header("Access-Control-Max-Age", String.valueOf(config.getCorsMaxAge()));
    }
